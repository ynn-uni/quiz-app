  ...
  <view class="past" :style="{'height':height+'px'}">
    <view class="title flex justify-between align-center">
      <view class="back" @click="beforeIssue">
        <text class="cuIcon-back"></text>上一期
      </view>
    ...
    <view
      ...
      :style="{'height':(height-23)+'px'}"
    >未获取到该期开奖数据</view>
    <view class="list" :style="{'height':(height-23)+'px'}">
      <view
        class="item flex align-center justify-center padding-tb-xs"
        v-for="(item,index) in lotteryList"
        :key="index"
      >
        <image class="margin-right-xs" :src="item.user.portrait" mode />
